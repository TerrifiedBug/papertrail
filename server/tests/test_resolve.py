...
def _evt(id, *, received_at, ttl=900, channel="home.status", kind="base", content=None):
    return EventRow(
        id=id,
        device="kitchen-01",
        channel=channel,
...
def test_newest_base_wins():
    now = 1000
    events = [
        _evt("old", received_at=900),
        _evt("new", received_at=950),
    ]
...
def test_live_interrupt_overlays_newer_base_then_expires():
    now = 1000
    events = [
        _evt("base", received_at=990, kind="base"),
        _evt("interrupt", received_at=900, ttl=200, kind="interrupt"),
    ]
...
def test_ttl_expiry_falls_back():
    ...
    events = [_evt("stale", received_at=900, ttl=900, kind="interrupt")]
    res = resolve_from_events(DEVICE, events, now)
...
def test_channel_not_subscribed_filtered():
    now = 1000
    events = [_evt("offchan", received_at=950, channel="home.other")]
    res = resolve_from_events(DEVICE, events, now)
